 ...
 confirmarPlazaBtn.addEventListener('click', async () => {
  if (plazaSeleccionada) {
    ...
    const data = {
      ...
      usuarioID: /* Obtener el id usuario, por ejemplo desde variable global o sesión */
    };
    ...
    try {
      ...
      if (result.success) {
        reservaForm.dataset.reservaID = result.reservaID; // Guarda ReservaID para el paso final
        plazasModal.style.display = 'none';
        reservaModal.style.display = 'block';
        alert("Reserva preliminar creada. Completa tu información para confirmar.");
      } else {
        alert("No se pudo reservar la plaza, intenta otra.");
      }
    } catch (error) {
      console.error("Error en reserva preliminar:", error);
